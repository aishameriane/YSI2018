...
2.  <span style="color:red">**Dúvida 2:**</span> Queria discutir sobre raizes unitárias...
...
5.  <span style="color:red">**Dúvida 5:**</span>
...
If mirror 10 ("UFRJ") is offline, try changing `ind = 10` to `ind = 1`. Do the same thing if you get an error installing `BETS` package.

```{r, warning = FALSE, message = FALSE}
chooseCRANmirror(graphics = FALSE, ind = 10)
if (!require("pacman")) install.packages("pacman")
pacman::p_load(devtools, ggplot2, forecast, BETS, seasonal, seasonalview, bvarsv, lubridate, zoo, stargazer, gridExtra, reshape2, ggfortify, RColorBrewer, scales, quantmod, PerformanceAnalytics, strucchange, coda, foreach, doParallel, knitr, grid, ggpubr, gdata, vars, urca) 
```

These are some auxiliary functions used later on the code (I took them from `bvarsv` documentation):
...
* **Taxa referencial de swaps DI pré-fixada (BM\&F) - Prazo de 90 dias (fim de período)** (_3-month threasury bill rate_)
...
This was the old block were I did the transformations prior to the seasonal adjustment. I'll let this here just in case but will build a new block where the data is adjusted for seasonality before the transformations.
...
autoplot(capital_trabalho)
ct.df <- ur.df(capital_trabalho, type = "drift", selectlags = "AIC", lags = 20)
summary(ct.df)
...
#ndiffs(ipca,test="adf",alpha = 0.05)
autoplot(ipca)
ipca.df <- ur.df(ipca, type = "drift", selectlags = "AIC", lags = 20)
summary(ipca.df)
# Mesma coisa, eu rejeito a hipótese nula a 1%?
...
# We test whether the GDP is non-stationary (via augmented dickey fuller test)
#ndiffs(PIBmensal,test="adf",alpha = 0.05)
autoplot(PIBmensal)
PIBm.df <- ur.df(PIBmensal, type = "trend", selectlags = "AIC", lags = 20)
summary(PIBm.df)
# Aqui eu não consigo rejeitar tau3 = 0 mas rejeito as outras duas?
# Since it is non-stationary, we take log difference
m <- seas(x = PIBmensal, regression.aictest = NULL)
qs(m)
summary(m)
PIBteste <- final(m)
autoplot(PIBteste)
PIBm.df <- ur.df(PIBteste, type = "trend", selectlags = "AIC", lags = 20)
summary(PIBm.df)

PIB <- diff(log(PIBteste), 1)
autoplot(PIB)
PIB.df <- ur.df(PIB, type = "none", selectlags = "AIC", lags = 20)
summary(PIB.df)
# Rejeito a hipótese nula de raiz unitária?
#ndiffs(PIB, test="adf", alpha = 0.05)
...
# Same thing we did for GDP
#ndiffs(cambio_raw,test="adf",alpha = 0.05)
autoplot(cambio_raw)
cr.df <- ur.df(cambio_raw, type = "trend", selectlags = "AIC", lags = 20)
summary(cr.df)
# Tô com dificuldade para interpretar quando o coeficiente é positivo... quando é MENOR eu rejeito a hipótese nula?
cambio     <- diff(log(cambio_raw), 1)
#ndiffs(cambio,test="adf",alpha = 0.05)
autoplot(cambio)
cambio.df <- ur.df(cambio, type = "none", selectlags = "AIC", lags = 20)
summary(cambio.df)
...
#ndiffs(selic, test="adf", alpha = 0.05)
autoplot(selic)
selic.df <- ur.df(selic, type = "none", selectlags = "AIC", lags = 20)
summary(selic.df)


swap90 <- BETS.get("7818", from = inicio_swap, to = fim)
#ndiffs(swap90, test="adf", alpha = 0.05)
autoplot(swap90)
swap.df <- ur.df(swap90, type = "trend", selectlags = "AIC", lags = 20)
summary(swap.df)
...
teste <- ur.df(diff(capital_trabalho_final), type = "drift", lags = 20, selectlags = "AIC")
summary(teste)
...
Ficou como estava. **Lembrar de colocar a análise depois**.

6. IPCA

Ficou como estava. **Lembrar de colocar a análise depois**.

# Novo Bloco de variáveis
...
#nburn.    <- 350000 # Burn the first 350k replications
...
p.        <- 1 # Number of lags
tau.      <- 48 # Number of observations to use as prior
...
#saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit2.rds") # Fit 2 I tried removing seasonality first on capital-labor
saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit500k.rds")
...
#Inflação - Produto - Juros - Câmbio - Desigualdade.
var1 <- cbind(ts(capital_trabalho_diff[1:(length(capital_trabalho_diff))], start=c(1996,02,01), frequency = 12)*100, ts(ipca[2:length(ipca)], start = c(1996, 02, 01), frequency = 12), ts(PIB[2:length(PIB)], start = c(1996, 02, 01), frequency = 12)*100, ts(swap[2:length(swap)], start = c(1996, 02, 01), frequency = 12), ts(cambio[2:length(cambio)], start = c(1996, 02, 01), frequency = 12)) # The bvar function does not allows data.frames
#var2 <- cbind(capital_trabalho_final, swap, PIB2, cambio, ipca) # The bvar function does not allows data.frames
names(var1) <- c("capital_trabalho", "ipca", "pib", "swap", "cambio")
#names(var1) <- c("ipca", "pib", "swap", "cambio","capital_trabalho") # Ficou uma merda
...
pm <- proc.time()
fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., thinfac = thinfac., nf = nf., k_B = k_B., k_A = k_A.)
saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit3.rds") # Fit 3 I tried removing seasonality first and took first diff on capital-labor
#saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit.rds")
fit1 <- readRDS("D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit3.rds")
#rm(fit1, ira)
proc.time() - pm
...
#Inflação - Produto - Juros - Câmbio - Desigualdade.
var1 <- cbind(capital_trabalho_diff*100, ts(ipca[2:length(ipca)], start = c(1996, 02, 01), frequency = 12), ts(PIB2[2:(length(PIB2)-1)], start = c(1996, 02, 01), frequency = 12)/10^4, ts(swap[2:length(swap)], start = c(1996, 02, 01), frequency = 12), ts(cambio[2:length(cambio)], start = c(1996, 02, 01), frequency = 12)) # The bvar function does not allows data.frames
#var2 <- cbind(capital_trabalho_final, swap, PIB2, cambio, ipca) # The bvar function does not allows data.frames
names(var1) <- c("capital_trabalho", "ipca", "pib", "swap", "cambio")
#names(var1) <- c("ipca", "pib", "swap", "cambio","capital_trabalho") # Ficou uma merda
...
pm <- proc.time()
fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., thinfac = thinfac., nf = nf., k_B = k_B., k_A = k_A.)
saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit4.rds") # Fit 4 i tried the PIB without first difference
fit1 <- readRDS("D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit4.rds")
#rm(fit1, ira)
proc.time() - pm
rm(fit1)
```
...
### Ipca na razão capital-trabalho
...
proc.time() - pm
saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fita novo.rds")
...
fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., nf = nf., thinfac = thinfac.)
saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fitb novo.rds")
...
#pdf(file = "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Figuras\\FIR\\Fig_brooks novo.pdf", width = 6, height = 3)
p2
#dev.off()
...
set.seed(2747)
#fit1 <- readRDS("D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit10for.rds")
fit1 <- bvar.sv.tvp(var1, p=p., tau = tau., nburn = nburn., nrep = nrep., nf = nf., thinfac = thinfac.)
saveRDS(fit1, "D:\\Onedrive - Aisha\\OneDrive\\Documentos\\Mestrado Economia\\Dissertação\\YSI - Buenos Aires\\Testes\\fit10for.rds")
#rm(fit1)
